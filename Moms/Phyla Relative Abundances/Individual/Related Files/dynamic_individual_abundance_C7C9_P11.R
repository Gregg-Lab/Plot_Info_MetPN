...
###Met PN Dam P11 Phyla Abundances
...
######ENTER IN VALUES######
TITLE <- "p_Proteobacteria Abundance Plot"
TAXONOMY_LEVEL <- "Phylum"
TAXA_ELEMENT <- "Proteobacteria"
TAXONOMY.FILE <- "combined_mompup.final.0.03.cons.taxonomy" #Cols: Size, Taxonomy
TIMEPOINT.FILE <- "combined_mompup.final.p11.shared" #Cols: OTU0001, OTU0002, etc.
DESIGN.FILE <- "combined_mom_P11.design.txt" #Cols: Sample, Group
###########################

#get into correct directory
setwd("~/Downloads/Microbiome-2019-07/Cohort_7_9_Mom_Pup/P11_IndividualPhylaAbundance/") #mac users
...
plots <- ggplot(data=selected_taxa_full, 
  ...
  geom_boxplot() +
  #facet_wrap(~taxa) +
  labs(title = "P11 p_Proteobacteria Relative Abundance Plot", y = "Relative Abundances (%)", x = "Groups") + 
  
  stat_compare_means(comparisons = list(c("CtrlPN", "MetPN")))
