...
setwd("~/Downloads/Microbiome-2019-07/Cohort_7_9_Mom_Pup/P11_BFRatio/") #for mac users
...
###GET PHYLUM###
TAX.LIST <- sort(unique(tax_clean$Phylum)) #9 including Bacteriaunclassified
...
otu.info <- otu.info[rowSums(otu.info) > 1000, ]  #removed two samples, 30B and 31B moms



#selects correct otus (> 200 total reads across samples)
otu.info <- otu.info[,names(otu.info) %in% tax_clean$OTU] #311 OTUs
