...
# VARIABLES #######################################################
TAXONOMY.FILE <- "combined_mompup.final.0.03.cons.taxonomy" #Cols: Size, Taxonomy
CORR.FILE <- "combined_mompup.final.pre.pearson.corr.axes" #Cols: OTU, axis1, p-value, axis2, p-value, length
PCOA.FILE <- "combined_mompup.final.pre.thetayc.0.03.lt.pcoa.axes" #Cols: group, axis1, axis2, etc.
DESIGN.FILE <- "combined_mom_PRE.design.txt" #Cols: Sample, Group
TIMEPOINT.FILE <- "combined_mompup.final.shared" #Cols: Otu0001, Otu0002, etc.
TITLE <- "Dam Pre-Pregnancy PCoA"
###################################################################

setwd("~/Downloads/Microbiome-2019-07/Cohort_7_9_Mom_Pup/PRE_PCoA_Mom/")
...
shape <- data.frame(c(rep(1, 7), rep(16, 4)))  #controls as open, and mets as filled
